...
// Calculate the amount of yaw <=> heading cap (with 0 as heading start)
...
void updateGyroData() {
  ...
  double dt = (double)(micros() - timer) / S_TO_US; // Calculate delta time
  timer = micros();
  ...
  // Integrate gyroZ through time to have a heading cap  
  gyroZangle += gyroZ/131.0*dt ;
